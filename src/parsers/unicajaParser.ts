...
/**
 * Parses a Unicaja XLS / XLSX sheet.
 */
export function parseUnicaja(sheet: XLSX.WorkSheet, sourceFileName: string): ParserOutput {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      if (cellVal === "número de cuenta:" || cellVal === "numero de cuenta:") {
        if (c + 1 < row.length) {
  ...
  // 2. Locate the header row by searching for a row whose first cell is "Fecha de operación"
  let headerRowIndex = -1;
  for (let r = 0; r < rows.length; r++) {
    const row = rows[r];
    if (row && row.length > 0 && String(row[0]).trim() === "Fecha de operación") {
      headerRowIndex = r;
      break;
    }
  }
  ...
  const idxOpDate = headers.indexOf("Fecha de operación");
  const idxValDate = headers.indexOf("Fecha valor");
  ...
  for (let r = headerRowIndex + 1; r < rows.length; r++) {
    ...
    // Validation: "Filas que no tengan una fecha de operación válida" are ignored.
